...
class MonnifyAPI(BaseRequestClient):
    ...
    def make_request(
        ...
        """
        Makes request to Klaviyo server for a url
        """
        headers = headers or {}
    ...
    def compute_auth(self):
        return TokenAuth(**{
            'custom_header_data': {
                'token': self.__authentication_token
            },
    ...
    def get_encoded_token(self):
        text = f'{self.monnify_api_key}:{self.monnify_secret_key}'
        byte_text = bytes(text, 'utf8')
    ...
    def compare_hash(self, hash_data, transaction_hash,):
        hash_data.update({
    ...
    def refresh_token(self, **kwargs):
        ...
        status_code, response_json = self.make_request(
            url_path, method=HttpMethods.POST,
            json_={},
        )
    ...
    def reserve_account(self, request_payload: dict) -> APIResultType:
        ...
        request_payload.update({
            "currencyCode": self.monnify_currency_code,
